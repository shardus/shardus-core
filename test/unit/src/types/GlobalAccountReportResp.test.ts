...
describe('GlobalAccountReportResp Tests', () => {
  ...
  test('Serialize and deserialize success response with empty accounts array', () => {
    const obj: GlobalAccountReportRespSerializable = {
      ready: true,
      combinedHash: 'hash123',
      accounts: [],
  ...
  test('Serialize and deserialize success response with null values', () => {
    const obj: GlobalAccountReportRespSerializable = {
      ready: false,
      combinedHash: '',
      accounts: [{ id: '', hash: '', timestamp: 0 }],
    }
    const stream = new VectorBufferStream(0)
    serializeGlobalAccountReportResp(stream, obj, false)
    stream.position = 0
  ...
  test('Deserialize success response with missing fields', () => {
    const stream = new VectorBufferStream(0)
    stream.writeUInt8(1)
    stream.writeUInt8(1)
    stream.writeString('hash123')
