...
describe('Sync Integration Tests', () => {
  ...
  describe('Sync.ts cache rebuild integration', () => {
    test('should rebuild cache after successful sync when enabled', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Rebuilt ProblematicNodeCache with 100 cycles after sync')
    })
    ...
    test('should rebuild cache after historical cycles sync', () => {
      ...
      const mockHistoricalSync = () => {
        if (historicalCycles.length > 0) {
          ...
          mockLogger.info(
            `syncV2: Historical sync complete - using network config value of ${Context.config.p2p.syncV2HistoricalCyclesCount}`
          )
          ...
          if (Context.config.p2p.enableProblematicNodeCacheBuilding) {
            ...
            if (availableHistory < requiredHistory) {
              mockLogger.warn(
                `syncV2: Insufficient cycle history for problematic node detection. Have ${availableHistory}, need ${requiredHistory}`
              )
            }
          }
        }
      ...
      expect(ProblemNodeHandler.rebuildCacheFromCycleChain).toHaveBeenCalled()
      expect(mockLogger.info).toHaveBeenCalledWith('syncV2: Rebuilt ProblematicNodeCache with 30 cycles')
    })
    ...
    test('should not rebuild cache when no historical cycles synced', () => {
      ...
      const mockHistoricalSync = () => {
        if (historicalCycles.length > 0) {
          ...
          mockLogger.info(
            `syncV2: No historical cycles synced (network config: ${Context.config.p2p.syncV2HistoricalCyclesCount})`
          )
        }
      }
      ...
      expect(ProblemNodeHandler.rebuildCacheFromCycleChain).not.toHaveBeenCalled()
      expect(mockLogger.info).toHaveBeenCalledWith('syncV2: No historical cycles synced (network config: 30)')
    })
  ...
  describe('error scenarios', () => {
    test('should handle cache rebuild errors gracefully', () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to rebuild cache after sync:', expect.any(Error))
    })
