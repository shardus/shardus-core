...
describe('select', () => {
  ...
  describe('selectNodes', () => {
    ...
    it('should cap maxAllowed to available nodes length', () => {
      const standbyNodesInfo = new Map([['key1', { selectionNum: '100', appJoinData: null }]])
  ...
  describe('notifyingNewestJoinedConsensors', () => {
    it('should notify joined consensors', async () => {
      ...
      expect(mockHttp.post).toHaveBeenCalledWith('http://192.168.1.1:9001/accepted', {
        signature: 'test-sig',
        sign: 'test-sign',
      })
      expect(mockHttp.post).not.toHaveBeenCalledWith('http://192.168.1.2:9002/accepted', expect.anything())
    })

    it('should handle http errors gracefully', async () => {
      const joinedConsensors = [{ publicKey: 'other-key', externalIp: '192.168.1.1', externalPort: 9001 }]
