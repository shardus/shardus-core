...
describe('ArchiverSyncTracker', () => {
  ...
  beforeEach(() => {
    ...
    (ArchiverDataSourceHelper as jest.MockedClass<typeof ArchiverDataSourceHelper>).mockImplementation(() => mockArchiverDataSourceHelper);
    
    // Mock getArchiversList
    (Archivers.getArchiversList as jest.Mock) = jest.fn().mockReturnValue([
      { ip: '127.0.0.1', port: 4000, publicKey: 'archiver1' }
    ]);
  ...
  describe('syncStateDataForRange2', () => {
    ...
    it('should handle debugFail3 error', async () => {
      ...
      expect(nestedCountersInstance.countEvent).toHaveBeenCalledWith('archiver_sync', 'syncStateDataForRange2: debugFail3');
    });
  ...
  describe('syncStateDataGlobals', () => {
    ...
    it('should sync global accounts successfully', async () => {
      ...
      expect(mockAccountSync.stateManager.writeCombinedAccountDataToBackups).toHaveBeenCalledWith(
        accountData,
        []
      );
      expect(mockAccountSync.setGlobalSyncFinished).toHaveBeenCalled();
  ...
  describe('syncAccountData2', () => {
    ...
    it('should sync account data successfully', async () => {
      ...
      const processAccountDataSpy = jest.spyOn(archiverSyncTracker, 'processAccountDataNoStateTable2').mockResolvedValue(2);
    ...
    it('should handle archiver busy response', async () => {
      ...
      expect(mockArchiverDataSourceHelper.tryNextDataSourceArchiver).toHaveBeenCalledWith(
    ...
    it('should handle pagination with offset', async () => {
      ...
      const batch2 = Array(500).fill(null).map((_, i) => ({
  ...
  describe('processAccountDataNoStateTable2', () => {
    ...
    it('should throw error when failed hashes exceed 1000', async () => {
      archiverSyncTracker.combinedAccountData = Array(1001).fill(null).map((_, i) => ({
        accountId: `acc${i}`,
        stateId: `hash${i}`,
        data: {},
        timestamp: 1000
      ...
      mockAccountSync.stateManager.checkAndSetAccountData.mockResolvedValue(
        Array(1001).fill(null).map((_, i) => `acc${i}`)
      );
  ...
  describe('tryRetry', () => {
    ...
    it('should throw error when max retries exceeded', async () => {
      ...
      expect(nestedCountersInstance.countEvent).toHaveBeenCalledWith('archiver_sync', 'tryRetry Out of tries test message');
    });
