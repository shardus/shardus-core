...
describe('CachedAppDataManager', () => {
  ...
  describe('constructor', () => {
    ...
    it('should set up pruning interval', () => {
      ...
      expect(setIntervalSpy).toHaveBeenCalledWith(
        expect.any(Function),
        mockConfig.p2p.cycleDuration * 1000
      )
  ...
  describe('insertCachedItem with size limits', () => {
    ...
    it('should not insert item exceeding maxItemSize', () => {
      ...
      expect(statemanagerFatalSpy).toHaveBeenCalledWith(
        'insertCachedItem',
        'Topic test-topic is at max size limit'
      )
  ...
  describe('factSendCorrespondingCachedAppData', () => {
    beforeEach(() => {
      ...
      ;(mockStateManager.transactionQueue.getConsenusGroupForAccount as jest.Mock)
        .mockImplementation((key: string) => {
          if (key === 'exec-key') return senderGroup
          return targetGroup
        })
      
      ;(mockStateManager.transactionQueue.getStartAndEndIndexOfTargetGroup as jest.Mock)
        .mockReturnValue({ startIndex: 3, endIndex: 5 })
      ...
      ;(mockStateManager.filterValidNodesForInternalMessage as jest.Mock)
        .mockImplementation((nodes) => nodes)
    })
    ...
    it('should handle edge case of sending to own node', () => {
      ...
      ;(mockStateManager.transactionQueue.getStartAndEndIndexOfTargetGroup as jest.Mock)
        .mockReturnValue({ startIndex: targetStartIndex, endIndex: targetEndIndex })
  ...
  describe('getLocalOrRemoteCachedAppData edge cases', () => {
    ...
    it('should throw error when network not ready after waiting', async () => {
      ...
      await expect(
        cachedAppDataManager.getLocalOrRemoteCachedAppData('test-topic', 'data-123')
      ).rejects.toThrow('getLocalOrRemoteCachedAppData: network not ready')
    })
