...
describe('GlobalAccounts', () => {
  ...
  describe('init', () => {
    it('should register internal binary handler and gossip handler', () => {
      ...
      expect(Comms.registerGossipHandler).toHaveBeenCalledWith('set-global', expect.any(Function))
    })
  })
  ...
  describe('setGlobal', () => {
    beforeEach(() => {
      ;(ShardFunctions.findHomeNode as jest.Mock).mockReturnValue({
        consensusNodeForOurNodeFull: [{ id: 'node1' }, { id: 'node2' }, { id: 'test-node-id' }, { id: 'node3' }],
      })
    })
    ...
    it('should create and broadcast signed transaction to consensus group', () => {
      ...
      expect(Self.emitter.on).toHaveBeenCalledWith(`receipt-${mockTxId}`, expect.any(Function))
    })
      ...
      ;(Self.emitter.on as jest.Mock).mockImplementation((event, handler) => {
        if (event === `receipt-${mockTxId}`) {
          ...
          setTimeout(() => {
            ...
            expect(Comms.sendGossip).toHaveBeenCalledWith('set-global', mockReceipt, '', null, NodeList.byIdOrder, true)
    ...
    it('should handle timeout when no receipt is received', (done) => {
      ...
      expect(Self.emitter.removeListener).toHaveBeenCalledWith(`receipt-${mockTxId}`, expect.any(Function))
  ...
  describe('makeReceipt', () => {
    beforeEach(() => {
      ;(ShardFunctions.findHomeNode as jest.Mock).mockReturnValue({
        consensusNodeForOurNodeFull: [{ id: 'node1' }, { id: 'node2' }, { id: 'node3' }],
      })
    })
  ...
  describe('setGlobalGossipHandler', () => {
    it('should validate and process valid receipt', () => {
      const mockReceipt = {
        signs: [{ owner: 'PUBKEY1' }, { owner: 'PUBKEY2' }],
        tx: {
          value: 'test-value',
          when: 1000,
          source: 'test-source',
      ...
      ;(ShardFunctions.findHomeNode as jest.Mock).mockReturnValue({
        consensusNodeForOurNodeFull: [{ id: 'node1' }, { id: 'node2' }],
      })
