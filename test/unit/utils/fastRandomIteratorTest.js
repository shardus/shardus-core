...
function unitTest(testSize, redunancy, passedRedundancy = -1) {
  ...
  console.log(
    `unit test passed: ${iterate.debugGetMode()}  array size:${testSize} picks:${redunancy}`
  )
}
...
function BuiltIn_ArrayShuffle() {
  const copy = randomTestList.slice() //have to copy the array to not mutate it! (alternative would be to create an array in indexes)
...
function FastRandomIterator_ForcedFast() {
  ...
  const iterate = new FastRandomIterator.default(perfTestArraySize, -1, -1)
  let nextIndex = 0
  let sum = 0
  for (let i = 0; i < sampleTestSize; i++) {
    nextIndex = iterate.getNextIndex()
...
function FastRandomIterator_ForcedFastSimple() {
  ...
  const iterate = new FastRandomIterator.default(perfTestArraySize, -1, 1000000)
  let nextIndex = 0
  let sum = 0
  for (let i = 0; i < sampleTestSize; i++) {
    nextIndex = iterate.getNextIndex()
...
function Random_LocalSparse() {
  ...
  const nextIndex = 0
  const arraySize = perfTestArraySize
  let sum = 0
  for (let i = 0; i < sampleTestSize; i++) {
    const iteratorIndex = i
...
function FastRandomIterator_Auto() {
  const iterate = new FastRandomIterator.default(
    perfTestArraySize,
    sampleTestSize
  )
  let nextIndex = 0
...
function PrimeNumberIterator() {
  //this only activates sparse some of the time..
  const arraySize = perfTestArraySize
  const P = primeLargerThan(arraySize)
  const N = perfTestArraySize
...
function RunPerfTests(arrayCount, arrayPicks) {
  ...
  const profiler = new Profiler.default()
  ...
  console.log(
    `\narraySize:${perfTestArraySize} numPicks:${sampleTestSize}  testIterationsPerType:${
      testLoops * 5
    }:`
  )
  ...
  const autoType = FastRandomIterator_GetAutoType()
