...
const ShardFunctions =
  require('../../../build/src/state-manager/shardFunctions2.js').default
...
function testPartitionMath1_old(numNodes, debugIndex) {
  let shardGlobals = ShardFunctions.calculateShardGlobals(
    numNodes,
  ...
  for (let i = 0; i < numNodes; i++) {
    ...
    let addressRange = ShardFunctions.partitionToAddressRange2_old(
      shardGlobals,
      partitionStart
    )

    let lowResult = ShardFunctions.addressToPartition_old(
      shardGlobals,
      addressRange.low
    )
    let highResult = ShardFunctions.addressToPartition_old(
      shardGlobals,
      addressRange.high
    )
    ...
    if (partitionStart !== lowResult.homePartition) {
      logErrorLimited(
        `failed partition ${partitionStart} low result: ${lowResult.homePartition}`
      )
    }
  }
  if (errorsLogged > 0) {
    console.log(
      `A num nodes: ${numNodes} complete total Errors: ${errorsLogged}`
    )
  }
}
...
function testPartitionMath1_new(numNodes, debugIndex) {
  ...
  for (let i = 0; i < numNodes; i++) {
    ...
    let addressRange = ShardFunctions.partitionToAddressRange2(
      shardGlobals,
      partitionStart
    )

    let lowResult = ShardFunctions.addressToPartition(
      shardGlobals,
      addressRange.low
    )
    let highResult = ShardFunctions.addressToPartition(
      shardGlobals,
      addressRange.high
    )

    let { address1, address2 } = ShardFunctions.getNextAdjacentAddresses(
      addressRange.high
    )
    ...
    if (nextAddress != null && nextAddress !== addressRange.low) {
      logErrorLimited(
        `failed getNextAdjacentAddresses ${nextAddress} addressRange.low: ${addressRange.low}`
      )
    }
  ...
  if (errorsLogged > 0) {
    console.log(
      `B num nodes: ${numNodes} complete total Errors: ${errorsLogged}`
    )
  }
}

function computePartitionShardDataMap(
  shardGlobals,
  partitionStart,
  partitionsToScan
) {
  ...
  for (let i = 0; i < partitionsToScan; ++i) {
    ...
    // increment index:
    let { homePartition, addressNum } = ShardFunctions.addressToPartition(
      shardGlobals,
      address
    )
    ...
    if (homePartition !== i) {
      logErrorLimited(
        `failed computePartitionShardDataMap ${homePartition} != ${i}`
      )
      return
    }
...
console.log(
  `--------unit test a hacked computePartitionShardDataMap--------------`
)
