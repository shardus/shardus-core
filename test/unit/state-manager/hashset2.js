...
function generateVariants(
  ...
  const results = new Array(numVariants + 2)
  const key = []

  const fullString = crypto.randomBytes(numElements * elementSizeInBytes)
  const dupeCheck = {}
  let dupeCount = 0
...
function loadVariantsFull(charsPerSlice, testHash, otherHashes, winningHash) {
  const results = new Array(3 + otherHashes.length + 2)
...
const useManualLoadSlice = true
const extendedValidation = true
let forcedIndex = -1

const log = false
const hashBytesSize = 2
...
for (let testIdx = 0; testIdx < testLoops; ++testIdx) {
  ...
  const solutionHashSet = /** @type {GenericHashSetEntry} */ {
    hash: 'solution',
    votePower: 1,
  ...
  const output = [] // StateManager.solveHashSets(hashSetList)
  profiler.profileSectionEnd('solveHashSets')

  // StateManager.solveHashSets(hashSetList)

  if (log) {
    for (const hashSetEntry of hashSetList) {
  ...
  const output2 = StateManager.solveHashSets2(hashSetList)
  profiler.profileSectionEnd('solveHashSets2')
  if (log) console.log(JSON.stringify(output2))

  const realResults = res.results[numVariants].join('')
  const oldRes = output.join('')
  const newRes = output2.join('')
  ...
  if (realResults === newRes) {
    ...
    if (extendedValidation) {
      for (let testIdx = 0; testIdx < numVariants; ++testIdx) {
        ...
        const result = StateManager.testHashsetSolution(
          hashSetList[testIdx],
          solutionHashSet
        )
        ...
        if (result) {
          ...
          const result2 = StateManager.testHashsetSolution(
            hashSetList[testIdx],
            solutionHashSet
          )
