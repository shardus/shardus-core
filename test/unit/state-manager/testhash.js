...
const accTest1obj = JSON.parse(accTest1str, utils.reviver)

const accTest1OldHash = accTest1obj.hash
accTest1obj.hash = ''
const accTest1Hash = crypto.hashObj(accTest1obj)

console.log(
  `accTest acc.hash=${accTest1OldHash} crypto.hashObj = ${accTest1Hash}`
)
...
function testParse(dir, filename) {
  let testInput = null
  try {
    const localDir = path.resolve(dir) // './test/unit/state-manager/')
    const data = fs.readFileSync(path.join(localDir, filename), 'utf8')
    // console.log(data)
...
const testInput = testParse(
  '../liberdus-server/instances12/',
  'shardCalcs2.txt'
)

const debugAccount = '38ab' + '3'.repeat(60)
