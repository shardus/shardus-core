...
export interface IPInfo {
  internalPort: number
  internalPortListen: number
  internalIp: string
  externalPort: number
  externalPortListen: number
  externalIp: string
}
...
export class NetworkClass extends EventEmitter {
  ...
  _setupExternal() {
    return new Promise((resolve, reject) => {
      ...
      this._applyExternal()
      this.extServer = this.app.listen(this.ipInfo.externalPortListen, () => {
        const msg = `External server running on port ${this.ipInfo.externalPortListen}...`
  ...
  async _setupInternal() {
    this.sn = Sn({
      port: this.ipInfo.internalPortListen,
    ...
    console.log(
      `Internal server running on port ${this.ipInfo.internalPortListen}...`
...
export async function init() {
  ...
  const externalPortListen = config.ip.externalPortListen === 0
      ? config.ip.externalPortListen
      : externalPort
  ...
  const internalPortListen = config.ip.internalPortListen === 0
      ? config.ip.internalPortListen
      : internalPort
