...
class AccountCache {
  ...
  updateAccountHash(accountId: string, hash: string, timestamp: number, cycle: number) {
    ...
    const onFutureCycle = cycle > this.accountsHashCache3.currentCalculationCycle

    if (accountHashCacheHistory.lastStaleCycle > 0 && accountHashCacheHistory.lastStaleCycle > accountHashCacheHistory.lastSeenCycle) {
      if (logFlags.verbose)
        this.mainLogger.debug(`Reinstate account c:${this.stateManager.currentCycleShardData.cycleNumber} acc:${utils.stringifyReduce(accountId)} lastStale:${accountHashCacheHistory.lastStaleCycle}`)
    }
    ...
    const accountHashData: AccountHashCache = { t: timestamp, h: hash, c: cycle }
    ...
    if (updateIsNewerHash === true && onFutureCycle === false) {
      const lastIndex = accountHashCacheHistory.lastSeenSortIndex
      ...
      const index = this.insertIntoHistoryList(accountId, accountHashData, this.accountsHashCache3.workingHistoryList, lastIndex)
      ...
      const index = this.insertIntoHistoryList(accountId, accountHashData, this.accountsHashCache3.futureHistoryList, lastIndex)
  ...
  insertIntoHistoryList(accountId: string, accountHashData: AccountHashCache, historyList: AccountHashCacheList, lastIndex: number): number {
    const accountHashesSorted: AccountHashCache[] = historyList.accountHashesSorted
    const accountIDs: string[] = historyList.accountIDs
  ...
  buildPartitionHashesForNode(cycleShardData: CycleShardData, debugAC3: AccountHashCacheMain3 | null = null, debugAccount: string | undefined = null): MainHashResults {
    ...
    const mainHashResults: MainHashResults = {
      cycle: cycleToProcess,
      partitionHashResults: new Map(),
    }

    const nextFutureList: AccountHashCacheList = {
      accountHashesSorted: [],
      accountIDs: [],
    }

    const tempList1: { id: string; t: number; entry: AccountHashCache }[] = []
    ...
    for (const key of this.accountsHashCache3.accountHashMap.keys()) {
      const accountCacheHistory = this.accountsHashCache3.accountHashMap.get(key)
      let index = 0
      ...
      if (index >= accountCacheHistory.accountHashList.length) {
        this.statemanager_fatal('buildPartitionHashesForNode: indexToohigh', `buildPartitionHashesForNode: indexToohigh :${index} `)
        continue
      }
  ...
  filterOutNonStoredData(mainHashResults: MainHashResults) {
    const cycle = mainHashResults.cycle
  ...
  buildPartitionHashesForNode_fast(cycleShardData: CycleShardData): MainHashResults {
    ...
    if (holes > 0) {
      ...
      const compactedWorkingList: AccountHashCacheList = {
        accountHashesSorted: [],
        accountIDs: [],
      }
      nextWorkingIndex = 0
