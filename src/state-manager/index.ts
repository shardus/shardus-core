...
class StateManager {
   ...
   *     ######   #######  ##    ##  ######     ##    ##     ##  #######   ######     ##     #######  ##     ##
   */
  constructor( profiler: Profiler, app: Shardus.App, logger: Logger, storage: Storage, p2p: P2P, crypto: Crypto, config: Shardus.StrictServerConfiguration) {
    //super()
    ...
    this.queueSitTime = 30000 // todo make this a setting. and tie in with the value in consensus
  ...
  tryStartAcceptedQueue() {
    ...
    if (!this.transactionQueue.newAcceptedTxQueueRunning) {
      txtryStartAcceptedQueue++
      this.mainLogger.info('Transaction Injected processAcceptedTxQueue: ',txtryStartAcceptedQueue)
