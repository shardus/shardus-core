...
export function getCycleNumberFromTimestamp(
  timestamp: number,
  allowOlder = true,
  addSyncSettleTime = true
) {
  const currentCycleShardData = stateManager.getCurrentCycleShardData()
  ...
  if (timestamp < 1 || timestamp === null) {
    ...
    stateManager.statemanager_fatal(
      ...
      `getCycleNumberFromTimestamp ${timestamp} ,  ${stack}`
    )
  }
  ...
  if (
    ...
    if (currentCycleShardData.cycleNumber === null) {
      ...
      nestedCountersInstance.countEvent(
        ...
        'currentCycleShardData.cycleNumber fail'
      )
      const cycle = getStoredCycleByTimestamp(offsetTimestamp)
      console.log(
        'CycleChain.getCycleNumberFromTimestamp getStoredCycleByTimestamp',
        cycle,
        timestamp
      )
      if (cycle !== null) {
        ...
        nestedCountersInstance.countEvent(
          'getCycleNumberFromTimestamp',
          'currentCycleShardData.cycleNumber redeemed'
        )
        return cycle.counter
      } else {
        //debug only!!!
        const cycle2 = getStoredCycleByTimestamp(offsetTimestamp)
        stateManager.statemanager_fatal(
          'getCycleNumberFromTimestamp failed fatal not redeemed',
          'getStoredCycleByTimestamp cycleNumber == null not redeemed'
        )
        nestedCountersInstance.countEvent(
          'getCycleNumberFromTimestamp',
          'currentCycleShardData.cycleNumber failed to redeem'
        )
      }
    } else {
  ...
  if (currentCycleShardData.cycleNumber === null) {
    ...
    stateManager.statemanager_fatal(
      'getCycleNumberFromTimestamp: currentCycleShardData.cycleNumber == null',
      `getCycleNumberFromTimestamp: currentCycleShardData.cycleNumber == null ${currentCycleShardData.cycleNumber} timestamp:${timestamp}`
    )
  }

  //is it in the future
  if (offsetTimestamp >= currentCycleShardData.timestampEndCycle) {
    const cycle: P2P.CycleCreatorTypes.CycleRecord = getNewest()

    const timePastCurrentCycle =
      offsetTimestamp - currentCycleShardData.timestampEndCycle
    const cyclesAhead = Math.ceil(
      timePastCurrentCycle / (cycle.duration * 1000)
    )
    nestedCountersInstance.countEvent(
      'getCycleNumberFromTimestamp',
      `+${cyclesAhead}`
  ...
  if (allowOlder === true) {
    ...
    if (cycle !== null) {
      ...
      if (cycle.counter === null) {
        ...
        nestedCountersInstance.countEvent(
          'getCycleNumberFromTimestamp',
          `getCycleNumberFromTimestamp unexpected cycle.cycleNumber == null  ${timestamp}`
        )
      }
      ...
      let cycleEstimate =
        ...
        Math.ceil(
          (currentCycleShardData.timestampEndCycle - offsetTimestamp) /
            (cycle.duration * 1000)
        )
      if (cycleEstimate < 1) {
        cycleEstimate = 1
      ...
      nestedCountersInstance.countEvent(
        'getCycleNumberFromTimestamp',
        'p2p lookup fail -estimate cycle: ' + cycleEstimate
      )
  ...
  stateManager.statemanager_fatal(
    'getCycleNumberFromTimestamp failed final',
    `getCycleNumberFromTimestamp failed final ${timestamp}`
  )
  return -1
}
