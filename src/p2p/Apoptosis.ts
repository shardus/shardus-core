...
const apoptosisInternalRoute: Route<InternalHandler<SignedApoptosisProposal>> =
  {
    ...
    handler: (payload, response, sender) => {
      ...
      if (!(payload as LooseObject).when) {
        response({ s: 'fail', r: 1 })
        return
      }
...
const apoptosisGossipRoute: GossipHandler<SignedApoptosisProposal> = (
  ...
  if ([1, 2].includes(currentQuarter) && addProposal(payload)) {
...
export function reset() {
  ...
  for (const id of Object.keys(proposals)) {
    if (!nodes.get(id)) delete proposals[id]
  }
}
...
*/
export function updateRecord(txs: Txs, record: Record) {
  const apoptosized = []
...
export function parseRecord(record: Record): Change {
  if (record.apoptosized.includes(Self.id)) {
    ...
    warn(
      `We got marked for apoptosis even though we didn't ask for it. Being nice and leaving.`
    )
...
export function sendRequests() {
  for (const id of Object.keys(proposals)) {
    ...
    if (nodes.get(id)) Comms.sendGossip(gossipRouteName, proposals[id])
  }
}
...
*/
  ...
  warn(
    'We have been apoptosized. Exiting with status 1. Will not be restarted.'
  )
}
