...
const lostReportRoute: P2P.P2PTypes.Route<
  P2P.P2PTypes.InternalHandler<P2P.LostTypes.SignedLostReport>
> = {
  name: 'lost-report',
...
export function getTxs(): P2P.LostTypes.Txs {
  ...
  for (const [key, obj] of lost) {
    ...
    if (
      obj.message &&
      obj.message.report &&
      obj.message.cycle === currentCycle
    ) {
...
export function validateRecordTypes(rec: P2P.LostTypes.Record): string {
  const err = validateTypes(rec, {lost: 'a', refuted: 'a'})
  if (err) return err
...
export function dropInvalidTxs(txs: P2P.LostTypes.Txs): P2P.LostTypes.Txs {
  const validLost = txs.lost.filter(
    request => checkDownMsg(request, currentCycle)[0]
  )
  const validRefuted = txs.refuted.filter(
    request => checkUpMsg(request, currentCycle)[0]
  )
...
export function sendRequests() {
  ...
  if (sendRefute > 0) {
    if (logFlags.p2pNonFatal)
      info(
        `sendRequest 5  sendRefute:${sendRefute}  currentCycle:${currentCycle}`
      )
  }
...
export function reportLost(target, reason) {
  ...
  const obj = {target: target.id, status: 'reported', cycle: currentCycle}
  ...
  if (logFlags.p2pNonFatal)
    info(
      `Sending investigate request: reporter:${Self.port} checker:${checker.externalPort} target:${target.externalPort} ` +
        JSON.stringify(msg)
    )
  msg = crypto.sign(msg)
  lost.set(key, obj)
...
function getCheckerNode(id, cycle) {
  ...
  if (logFlags.p2pNonFatal) {
    info(
      `in getCheckerNode oidx:${oidx} idx:${idx} near:${near}  cycle:${cycle}  id:${id}`
    )
...
async function lostReportHandler(payload, response, sender) {
  ...
  try {
    if (logFlags.p2pNonFatal)
      info(
        `Got investigate request: ${JSON.stringify(
          payload
    ...
    const obj: P2P.LostTypes.LostRecord = {
...
function checkReport(report, expectCycle) {
  ...
  if (report.target === report.reporter)
    return [false, 'target cannot be reporter'] // the target should not be the reporter
  if (report.checker === report.target)
    return [false, 'target cannot be checker'] // the target should not be the reporter
  ...
  const checkerNode = getCheckerNode(report.target, report.cycle)
...
function downGossipHandler(
  ...
  err = validateTypes(payload.report, {
    target: 's',
    reporter: 's',
    checker: 's',
    cycle: 'n',
...
function upGossipHandler(payload, sender, tracker) {
  ...
  if (!valid) {
    warn(
      `Bad upGossip message. reason:${reason} message:${JSON.stringify(
        payload
      )}`
    )
    return
...
function checkUpMsg(
  ...
  if (!nodes.has(payload.target))
    return [
      false,
      `target is not an active node  ${payload.target}  ${JSON.stringify(
        activeByIdOrder
      )}`,
    ]
