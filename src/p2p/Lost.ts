...
export function getTxs(): P2P.LostTypes.Txs {
  ...
  for (const [key, lostRecordItems] of receivedLostRecordMap) {
    ...
    nestedCountersInstance.countEvent('testingLost', `${currentCycle}: down count: ${downMsgCount}, up count: ${upMsgCount}`)
    ...
    if (upMsgCount >= config.p2p.minChecksForUp) {
      ...
      nestedCountersInstance.countEvent('testingLost', `${currentCycle}: Saw at least ${config.p2p.minChecksForUp} up messages`)
      ...
      seen[downRecord.target] = true
      nestedCountersInstance.countEvent('testingLost', `${currentCycle}: Adding lost record for ${downRecord.target} to lostTxs`)
      ...
      /* prettier-ignore */ if (logFlags.lost) info(`Not enough down messages to be considered lost: ${JSON.stringify(downRecord)}`)
    }
  }
  ...
  if (config.p2p.detectLostSyncing) {
    ...
    for (const syncingNode of syncingNodes) {
      ...
      if (record.maxSyncTime && syncTime > record.maxSyncTime) {
        /* prettier-ignore */ if (logFlags.lost) info(`Syncing time for node ${syncingNode.id}`, syncTime)
        /* prettier-ignore */ if (logFlags.lost) info(`Max sync time from record`, record.maxSyncTime)
        /* prettier-ignore */ if (logFlags.lost) info(`Sync time is longer than max sync time. Reporting as lost`)
        /* prettier-ignore */ if (logFlags.lost) info('adding node to lost syncing list', syncingNode.id, `${syncTime} > ${record.maxSyncTime}`)
...
export function scheduleLostReport(target: P2P.NodeListTypes.Node, reason: string, requestId: string) {
  ...
  /* prettier-ignore */ if (logFlags.lost) info(`Scheduling lost report for ${target.id}, requestId: ${requestId}.`)
  /* prettier-ignore */ if (logFlags.lost) info(`Target node details for requestId: ${requestId}: ${logNode(target)}`)
  /* prettier-ignore */ if (logFlags.lost) info(`Scheduled lost report in ${currentCycle} for requestId: ${requestId}.`)

  /* prettier-ignore */ if (logFlags.lost) console.log(`scheduleLostReport: key: ${target.id}-${currentCycle}`)
...
function reportLost(target, reason: string, requestId: string) {
  ...
  try {
    ...
    for (let checker of checkerNodes) {
      ...
      /* prettier-ignore */ if (logFlags.lost) info(`Sending investigate request. requestId: ${requestId}, reporter: ${Self.ip}:${Self.port} id: ${Self.id}`)
      /* prettier-ignore */ if (logFlags.lost) info(`Sending investigate request. requestId: ${requestId}, checker: ${checker.internalIp}:${checker.internalPort} node details: ${logNode(checker)}`)
      /* prettier-ignore */ if (logFlags.lost) info(`Sending investigate request. requestId: ${requestId}, target: ${target.internalIp}:${target.internalPort} cycle: ${report.cycle} node details: ${logNode(target)}`)
      /* prettier-ignore */ if (logFlags.lost) info(`Sending investigate request. requestId: ${requestId}, msg: ${JSON.stringify(report)}`)
...
function getCheckerNode(id, cycle) {
  ...
  /* prettier-ignore */ if (logFlags.lost) info(`in getCheckerNode oidx:${oidx} idx:${idx} near:${near}  cycle:${cycle}  id:${id}`)
  /* prettier-ignore */ if (logFlags.lost) info(`${JSON.stringify(activeByIdOrder.map((n) => n.id))}`)
...
async function lostReportHandler(payload, response, sender) {
  ...
  try {
    ...
    /* prettier-ignore */ if (logFlags.lost) info(`Got investigate request requestId: ${requestId}, req: ${JSON.stringify(payload)} from ${logNode(sender)}`)
    let err = ''
    // for request tracing
...
function downGossipHandler(payload: P2P.LostTypes.SignedDownGossipMessage, sender, tracker) {
  ...
  /* prettier-ignore */ if (logFlags.lost) info(`Got downGossip: ${JSON.stringify(payload)}`)
  let err = ''
...
function upGossipHandler(payload, sender, tracker) {
  /* prettier-ignore */ if (logFlags.lost) info(`Got upGossip: ${JSON.stringify(payload)}`)
  let err = ''
