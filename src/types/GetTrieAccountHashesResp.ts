...
export function serializeGetTrieAccountHashesResp(stream: VectorBufferStream, req: HashTrieAccountsResp, root = false): void {
  if(root) {
...
export function deserializeGetTrieAccountHashesResp(stream: VectorBufferStream): HashTrieAccountsResp {
...
export function deserializeRadixAndChildHashes(stream: VectorBufferStream): RadixAndChildHashes {
  ...
  let childAccounts: AccountIDAndHash[] = [];
