...
export function propComparator<T>(prop: keyof T): (a: T, b: T) => 0 | 1 | -1 {
  // eslint-disable-next-line security/detect-object-injection
  const comparator = (a: T, b: T): 0 | 1 | -1 => (a[prop] > b[prop] ? 1 : a[prop] < b[prop] ? -1 : 0)
  return comparator
}

export function propComparator2<T>(prop: keyof T, prop2: keyof T): (a: T, b: T) => 0 | 1 | -1 {
  /* eslint-disable security/detect-object-injection */
  const comparator = (a: T, b: T): 0 | 1 | -1 =>
    a[prop] === b[prop]
      ? a[prop2] === b[prop2]
        ? 0
        : a[prop2] > b[prop2]
