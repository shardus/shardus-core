...
function analyzeCache(cacheData) {
    ...
    const totalRefutes = cacheData.refuteHistory ? 
        Object.values(cacheData.refuteHistory).reduce((sum, cycles) => sum + cycles.length, 0) : 0;
...
async function main(archiverUrl) {
    ...
    console.log(`\r${colors.green}Successfully retrieved cache from ${successCount}/${nodes.length} nodes${colors.reset}\n`);
    ...
    if (problematicNodesData) {
        ...
        console.log(`Eviction enabled: ${problematicNodesData.evictionEnabled ? colors.green + 'Yes' : colors.red + 'No'}${colors.reset}`);
        console.log(`\nThresholds:`);
        console.log(`  - Consecutive refutes: ${colors.yellow}${problematicNodesData.thresholds.consecutiveRefutes}${colors.reset}`);
        console.log(`  - Refute percentage: ${colors.yellow}${problematicNodesData.thresholds.refutePercentage * 100}%${colors.reset}`);
        console.log(`  - History length: ${colors.yellow}${problematicNodesData.thresholds.historyLength} cycles${colors.reset}`);
        ...
        if (problematicNodesData.problematicNodes.length > 0) {
            ...
            for (const node of problematicNodesData.problematicNodes) {
                console.log(`  - Node ${colors.yellow}${node.id}${colors.reset}: ${node.consecutiveRefutes} consecutive refutes, ${node.refutePercentage}% refute rate`);
            }
        }
        console.log();
    }
    ...
    for (const result of results) {
        ...
        if (analysis) {
            ...
            if (analysis.cycleRange) {
                console.log(`    - Cycle range: ${colors.cyan}${analysis.cycleRange.min} - ${analysis.cycleRange.max}${colors.reset}`);
                ...
                console.log(`    - Cycle coverage: ${colors.green}${coverage}%${colors.reset} (${analysis.totalCyclesProcessed}/${expectedCycles})`);
            }
            
            if (analysis.missingCycles && analysis.missingCycles.length > 0) {
                console.log(`    - ${colors.red}Missing cycles: ${analysis.missingCycles.slice(0, 5).join(', ')}${analysis.missingCycles.length > 5 ? '...' : ''}${colors.reset}`);
            }
        } else {
    ...
    if (validResults.length >= 2) {
        ...
        if (discrepancies.length > 0) {
            ...
            for (const disc of discrepancies) {
                ...
                console.log(`  - Nodes with different refutes: ${colors.yellow}${disc.comparison.differentRefutes}${colors.reset}`);
            }
        }
    }
