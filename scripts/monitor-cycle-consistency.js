    ...
    for (const endpoint of endpoints) {
        ...
        if (data) {
            ...
            if (nodes && nodes.length > 0) {
                ...
                return formattedNodes.filter(node => 
...
async function getActiveNodes(archiverUrl) {
    ...
    if (!activeValidator) {
        console.log(`  ${colors.yellow}Warning: No active validators found in archiver list, using archiver list anyway${colors.reset}`);
    ...
    while (true) {
        ...
        const nodePromises = nodes.map(async (node) => {
            ...
            try {
                ...
                const [inventory, chainHash] = await Promise.all([
                    getCycleInventory(node),
                    getCycleChainHash(node)
                ]);
        ...
        console.log(`  Completed in ${colors.yellow}${elapsedTime}s${colors.reset} - Success: ${colors.green}${inventories.length}${colors.reset}, Failed: ${colors.red}${failedNodes.length}${colors.reset}`);
        ...
        if (inventories.length === 0) {
            ...
            console.log(`  Average cycles stored: ${colors.yellow}${avgStoredCycles.toFixed(1)}/${MAX_CYCLES_TO_KEEP}${colors.reset}`);
            ...
            if (chainHashes.length > 0) {
                ...
                if (chainHashes.length === inventories.length) {
                    ...
                    Object.values(nodesByRange).forEach(range => {
                        if (range.end === currentCycle && range.nodes.length > 1) {
                            ...
                            const nodesWithSameHash = chainHashes.filter(ch => 
                                ch.cycleRange.end === range.end && 
                                ch.cycleRange.start === range.start && 
                                ch.chainHash === range.hash
                            ).length;
                    ...
                    if (commonRecentCycles === 0 && minStoredCycles > 0) {
                        ...
                        if (allRangesIdentical) {
                            ...
                            const nodesWithThisCycle = chainHashes.filter(ch => 
                                ch.cycleRange.start <= newestNodeStart && 
                                ch.cycleRange.end >= newestNodeStart
                            );
                ...
                if (trueCommonHistory > 0) {
                    console.log(`  ${colors.bright}${colors.green}✓ Most recent ${trueCommonHistory} cycles are identical across all nodes${colors.reset}`);
                    ...
                    if (trueCommonHistory < minStoredCycles) {
                        ...
                        console.log(`  ${colors.cyan}ℹ Newest node only has ${minStoredCycles} cycles (joined at cycle ${currentCycle - minStoredCycles + 1})${colors.reset}`);
                    }
                } else {
                ...
                sortedRanges.forEach(([range, nodes]) => {
                    ...
                    if (uniqueHashes.length > 1) {
                        ...
                        console.log(`  ${colors.red}✗ Cycles ${range} (${cycleCount} cycles, ${nodes.length} nodes): ${uniqueHashes.length} different versions${colors.reset}`);
                        ...
                        console.log(`  ${colors.green}✓ Cycles ${range} (${cycleCount} cycles, ${nodes.length} nodes): Identical (${nodes[0].hash}...)${colors.reset}`);
                    }
                });
            } else {
            ...
            if (failedNodes.length > 0) {
                ...
                const otherFailures = failedNodes.filter(node => 
                    !inactiveNodes.some(inactive => inactive.nodeId === node.nodeId)
                );
